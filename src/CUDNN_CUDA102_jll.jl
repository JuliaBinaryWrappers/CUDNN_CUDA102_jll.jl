...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import CUDNN_CUDA102_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("CUDNN_CUDA102_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "CUDNN_CUDA102_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"CUDNN_CUDA102", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "CUDNN_CUDA102_jll", "src", "CUDNN_CUDA102_jll.jl"))
        @info("CUDNN_CUDA102_ll dev'ed out to /depot/dev/CUDNN_CUDA102_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("8ba91e8f-b0e5-5ca0-b631-aadbb9431ebf"))
platforms = [Pkg.Artifacts.unpack_platform(e, "CUDNN_CUDA102", artifacts_toml) for e in artifacts["CUDNN_CUDA102"]]
...
if best_platform === nothing
    @debug("Unable to load CUDNN_CUDA102; unsupported platform $(triplet(platform_key_abi()))")
else
